...
local function safeInvoke(remote, ...)
    task.wait(math.random() * 0.2 + 0.1)  -- Random delay to mimic normal behavior
...
local function spawnItem(itemName, position, orientation)
    task.spawn(function()
        ...
        safeInvoke(ReplicatedStorage.MenuToys.SpawnToyRemoteFunction, itemName, cframe, rotation)
    end)
end
...
local function fireAll()
    while true do
        local success, err = pcall(function()
            ...
            safeFire(SetNetworkOwner, firePlayerPart, firePlayerPart.CFrame)
                        ...
                        if partModel then
                            ...
                            if distance <= 30 then
                                ...
                                if highlight and highlight.OutlineColor == Color3.new(1, 0, 0) then
                                    safeFire(SetNetworkOwner, partModel, partModel.CFrame)
                ...
                connectionBombReload = toysFolder.ChildAdded:Connect(function(child)
                    if child.Name == _G.ToyToLoad and child:WaitForChild("ThisToysNumber", 1) then
                        if child.ThisToysNumber.Value == (toysFolder.ToyNumber.Value - 1) then
                            ...
                            safeFire(SetNetworkOwner, child.Body, child.Body.CFrame)
...
local function blobGrabPlayer(player, blobman)
    if blobalter == 1 then
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            safeFire(blobman:WaitForChild("BlobmanSeatAndOwnerScript"):WaitForChild("CreatureGrab"), unpack(args))
            blobalter = 2
        end
    else
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if enabled then
            autoStruggleCoroutine = RunService.Heartbeat:Connect(function()
                ...
                if character and character:FindFirstChild("Head") then
                    ...
                    if partOwner then
                        safeFire(Struggle)
                        safeFire(ReplicatedStorage.GameCorrectionEvents.StopAllVelocity)
                        for _, part in pairs(character:GetChildren()) do
                            if part:IsA("BasePart") then
                                part.Anchored = true
                            end
            ...
            antiKickCoroutine = RunService.Heartbeat:Connect(function()
                ...
                if character and character:FindFirstChild("HumanoidRootPart") and character:FindFirstChild("HumanoidRootPart"):FindFirstChild("FirePlayerPart") then
                    ...
                    if partOwner and partOwner.Value ~= localPlayer.Name then
                        ...
                        safeFire(game:GetService("ReplicatedStorage"):WaitForChild("CharacterEvents"):WaitForChild("RagdollRemote"), unpack(args))
                        print("grabbity shap!")
                        wait(0.1)
                        safeFire(Struggle)
                    end
                    ...
                    if character and character:FindFirstChild("Head") then
                        ...
                        if partOwner then
                            ...
                            if attacker and attacker.Character then
                                safeFire(Struggle)
                                safeFire(SetNetworkOwner, attacker.Character.Head or attacker.Character.Torso, attacker.Character.HumanoidRootPart.FirePlayerPart.CFrame)
                                task.wait(0.1)
    ...
    Callback = function()
        ...
        local function setupDecoy(decoy)
            ...
            if torso then
                ...
                safeFire(SetNetworkOwner, torso, playerCharacter.Head.CFrame)
            end
        end
